...
**날씨를 정보가 아니라 하루의 준비물로 번역하는 앱**입니다.

`28℃` 를 알려 주는 앱은 이미 많습니다. 이 앱은 다른 질문에 답합니다 —
오늘 뭐 입지, 오늘 뭐 듣지, 지금 여기는 어떤지.

현재 위치에서 시작해서, 기온에 맞는 옷차림을 추천하고, 그날 하늘에 어울리는
플레이리스트를 틀어 줍니다. 보고 싶은 지역은 검색으로 얼마든지 담을 수 있습니다.
...
| API | 하는 일 |
|:---|:---|
| OWM `/weather` · `/forecast` | 현재 날씨 · 시간별 / 일별 예보 |
| OWM `/geo/1.0/direct` | 지역 이름 → 좌표 (검색으로 담기) |
| OWM `/geo/1.0/reverse` | 좌표 → 지명 (GPS) |
...
| 기능 | 설명 |
|:---|:---|
| 📍 지역 검색 · GPS | 목록에 없는 지역을 치면 Geocoding 으로 찾아 담습니다. `⊙ 내 위치` 로 지금 있는 곳도 담을 수 있고, 앱을 켜면 현재 위치에서 가장 가까운 도시부터 보여 줍니다 |
| ⭐ 즐겨찾기 · 정렬 | 별을 누른 순서가 곧 목록 순서입니다. 다시 담으면 맨 앞으로 옵니다 |
...
| 🌗 스플래시 · 테마 | 첫 방문에는 소개 문구가 한 글자씩 찍힙니다(타이프라이터). 다크 / 라이트는 OS 설정을 따라갑니다 |
| 📱 모바일 | 좁은 화면에서는 카드를 6장만 보여 주고 접습니다. 페이지 안에 또 스크롤을 만들지 않았습니다 |
